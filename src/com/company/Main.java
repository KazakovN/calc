...
public class Main {
        ...
        static void iivString(String omegaString)
        {
            ...
            for (int i=0; i<p; i++)
            {
                if (omegaString.charAt(i) == '+') index=omegaString.indexOf('+');
                if (omegaString.charAt(i) == '-')index=omegaString.indexOf('-');
                if (omegaString.charAt(i) == '/') index=omegaString.indexOf('/');
                if (omegaString.charAt(i) == '*')index=omegaString.indexOf('*');
            }
            ...
            {
                omegaString= String.valueOf(x) +omegaString.charAt(index)+ y;
        ...
        static void convertString(String omegaString,boolean b)
        {
            ...
            for (int j=0; j<omegaString.length(); j++) {
                switch (omegaString.charAt(j)) {
                    case '+' -> inOutStr(addOp(x, y), b);
                    case '-' -> inOutStr(subOp(x, y), b);
                    case '/' -> inOutStr(divOp(x, y), b);
                    case '*' -> inOutStr(mulOp(x, y), b);
                }
            }
        }
